...
class ZoomAccountController extends Controller
{
    public function index(): Response
    {
        $accounts = ZoomAccount::query()
            ...
            ->latest()
            ->get(['id', 'name', 'slug', 'account_id', 'client_id', 'status', 'created_at', 'updated_at']);

        return Inertia::render('admin/zoom-accounts/index', [
    ...
    {
        return Inertia::render('admin/zoom-accounts/show', [
    ...
    {
        $zoomAccount->update(['status' => 'inactive']);
    ...
    private function capacitySummary($accounts): array
    {
        $fullAccounts = $accounts
            ->filter(fn (ZoomAccount $account): bool => $account->status === 'active')
    ...
    {
        $activeBookings = $account->schedules
